...
class TestPluginsFileOperations < Minitest::Test
  ...
  def test_file_operations_plugin_exists
    assert defined?(Beniya::Plugins::FileOperations),
           "Beniya::Plugins::FileOperations クラスが定義されていません"
  end

  def test_file_operations_inherits_from_plugin
    assert @plugin.is_a?(Beniya::Plugin),
           "FileOperations は Beniya::Plugin を継承している必要があります"
  end
  ...
  def test_has_no_dependencies
    # 外部gem依存なし
    assert_equal [], Beniya::Plugins::FileOperations.required_gems
  end
  ...
  def test_plugin_is_registered_in_plugin_manager
    ...
    plugin_classes = Beniya::PluginManager.plugins
    assert_includes plugin_classes, Beniya::Plugins::FileOperations
  end
end
