...
require_relative '../lib/beniya/logger'
require 'tempfile'

class TestLogger < Minitest::Test
  def setup
    @original_debug_env = ENV['BENIYA_DEBUG']
    @original_log_file = Beniya::Logger::LOG_FILE

    # Create a temporary log file for testing
    @temp_log_file = Tempfile.new(['beniya_test_log', '.log'])
    @temp_log_path = @temp_log_file.path
    @temp_log_file.close

    # Override LOG_FILE constant for testing (remove first to avoid warning)
    Beniya::Logger.send(:remove_const, :LOG_FILE) if Beniya::Logger.const_defined?(:LOG_FILE)
    Beniya::Logger.const_set(:LOG_FILE, @temp_log_path)
  end
  ...
  def teardown
    ...
    Beniya::Logger.send(:remove_const, :LOG_FILE) if Beniya::Logger.const_defined?(:LOG_FILE)
    Beniya::Logger.const_set(:LOG_FILE, @original_log_file) if @original_log_file
  ...
  def test_debug_disabled_by_default
    ...
    Beniya::Logger.debug('Test message')
  ...
  def test_debug_logs_when_enabled
    ...
    Beniya::Logger.debug('Test debug message')
  ...
  def test_debug_with_context
    ...
    Beniya::Logger.debug('Operation started', context: { file: 'test.txt', count: 5 })
  ...
  def test_info_logs_when_enabled
    ...
    Beniya::Logger.info('Info message')
  ...
  def test_warn_logs_when_enabled
    ...
    Beniya::Logger.warn('Warning message')
  ...
  def test_error_logs_when_enabled
    ...
    Beniya::Logger.error('Error occurred')
  ...
  def test_error_with_exception
    ...
    begin
      ...
      Beniya::Logger.error('Exception caught', exception: e)
    end
  ...
  def test_error_with_context_and_exception
    ...
    begin
      ...
      Beniya::Logger.error('Operation failed', exception: e, context: { operation: 'delete', file: 'test.txt' })
    end
  ...
  def test_clear_log
    ...
    # Write some logs
    Beniya::Logger.debug('First message')
    Beniya::Logger.debug('Second message')

    # Clear the log
    Beniya::Logger.clear_log

    content = File.read(@temp_log_path)
    refute_includes content, 'First message'
    refute_includes content, 'Second message'
    assert_includes content, 'Beniya Debug Log Cleared'
  end

  def test_timestamp_format
    ENV['BENIYA_DEBUG'] = '1'

    Beniya::Logger.debug('Test timestamp')
  ...
  def test_multiple_log_entries
    ...
    Beniya::Logger.debug('First entry')
    Beniya::Logger.info('Second entry')
    Beniya::Logger.warn('Third entry')
    Beniya::Logger.error('Fourth entry')
  ...
  def test_large_context_array_truncation
    ...
    Beniya::Logger.debug('Large array test', context: { items: large_array })
  ...
  def test_log_file_append_mode
    ...
    Beniya::Logger.debug('First log')
    first_content = File.read(@temp_log_path)

    Beniya::Logger.debug('Second log')
