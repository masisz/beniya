...
class DeleteTimingTest
  def initialize
    @test_dir = Dir.mktmpdir("beniya_delete_timing_test")
  ...
  def simulate_beniya_delete_logic_single_file
    puts "\n=== Beniya削除ロジックシミュレーション（単一ファイル） ==="
    ...
    files.each do |filename|
      ...
      begin
        ...
        # 削除後の存在確認（Beniyaのロジックと同じ）
        sleep(0.01) # 10ms待機（Beniyaと同じ）
  ...
  def simulate_beniya_delete_logic
    puts "\n=== Beniya削除ロジックシミュレーション（複数ファイル） ==="
  ...
  def run_all_tests
    ...
    simulate_beniya_delete_logic_single_file
    simulate_beniya_delete_logic
  end
end
