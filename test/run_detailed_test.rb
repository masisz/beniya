...
class DetailedTest
  def initialize
    @test_dir = Dir.mktmpdir("beniya_test")
  ...
  def test_directory_listing_initialization
    listing = Beniya::DirectoryListing.new(@test_dir)
    if listing.is_a?(Beniya::DirectoryListing) && listing.current_path == @test_dir
  ...
  def run_all_tests
    puts "=== Beniya DirectoryListing 詳細テスト開始 ==="
