...
# 実際のbeniya実行時のメモリ使用量を測定
# このスクリプトは別プロセスでbeniyaを起動して測定します
...
class RealUsageBenchmark
  ...
  def self.run
    puts "=" * 70
    puts "beniya Real Usage Memory Benchmark"
    puts "=" * 70
    puts
  ...
  def self.create_test_scenarios
    ...
    small_dir = Dir.mktmpdir("beniya_small")
    ...
    # 2. Medium directory
    medium_dir = Dir.mktmpdir("beniya_medium")
    ...
    # 3. Large directory
    large_dir = Dir.mktmpdir("beniya_large")
  ...
  def self.measure_component_memory(path)
    ...
    directory_listing = Beniya::DirectoryListing.new(path)
  ...
  def self.show_summary
    puts
    puts "Key Findings:"
    puts "  • Base memory usage: ~20-25 MB (Ruby runtime + beniya code)"
    ...
    puts "  • nnn (C): ~5-10 MB"
    puts "  • lf (Go): ~10-20 MB"
    puts "  • beniya (Ruby): ~20-35 MB ← You are here"
    puts
    puts "beniya is reasonably lightweight for a Ruby application!"
  end
end
...
def measure_allocations
  ...
  test_dir = Dir.mktmpdir("beniya_alloc")
