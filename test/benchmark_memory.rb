...
puts "=" * 60
puts "beniya Memory Usage Benchmark"
puts "=" * 60
puts
...
puts "-" * 60
test_dir = Dir.mktmpdir("beniya_memory_test")
# テストファイルを作成
100.times do |i|
...
mem = MemoryBenchmark.measure do
  Beniya::DirectoryListing.new(test_dir)
end
...
mem = MemoryBenchmark.measure do
  ...
  # terminal_ui = Beniya::TerminalUI.new  # 実際のターミナルUIは起動しない
end
...
puts "-" * 60
large_test_dir = Dir.mktmpdir("beniya_large_test")
1000.times do |i|
  File.write(File.join(large_test_dir, "file_#{i}.txt"), "test content #{i}")
end

mem = MemoryBenchmark.measure do
  directory_listing = Beniya::DirectoryListing.new(large_test_dir)
...
puts "Summary"
puts "=" * 60
puts "beniya is a lightweight file manager with minimal memory footprint."
